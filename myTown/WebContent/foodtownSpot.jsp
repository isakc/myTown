...
<%
	...
	//메뉴가격 평균
	int price = MenuDAO.selectMenuPrice(no);
	//메뉴
	List<MenuVo> menu = MenuDAO.selectMenu(no);
	
	//댓글 페이징 처리
	PageVo pageVo = new PageVo(now, numPage,no);
	String paging = PaginateUtil.paginate(now, total, numPage, numBlock, "1foodtownSpot.jsp","no="+ no +"&now=");
	//댓글 받아오기
...
<div id="wrap">
		...
		<div id="top"><!-- top 시작 -->
		<img id="category" src="img/foodTown.png" alt="1차카테고리"/>
	...
	<div id="bottom">
		<div id="info">
			...
			<hr/>
			<% String deliveryPossible = spot.getDelivery();
			   String delivery;
				if(deliveryPossible == "t") {
					delivery = "가능";
				}else if(deliveryPossible == "f"){
					delivery = "불가능";
				}else {
					delivery = "정보 없음";
				}
			...
			<div class="spotBasic">배달:<span><%=delivery %></span></div>
			</div>
		</div>
		...
		<h3 id="hashtagTitle">'<%=spot.getName() %>' 관련 해시태그</h3>
			<div id="tagCloud">
                        <ul>
                            <li class="c5 cate2"><a href=""><%if(hashtags.size()==0){%><%="태그가 없어요!"%>
                            <%}else{%><%=hashtags.get(0).getTag() %><%} %></a></li>
		...
		<div id="menuBox">
			<h3>MENU</h3>
			<div id="menus">
				<h4>평균가격 <span><%=price %>원</span></h4>
					<ul class="menuList">
					<%for(MenuVo menuOne : menu ){ %>
						<li class="menu">
							<span class="mN"><%=menuOne.getName() %></span> <span class="mP"><%=menuOne.getPrice() %>원</span>
						</li>
					<%} %>
					</ul>
				<div id="addBtn">메뉴를 추가하고 싶으세요?</div>
				<div id="menuAddBox" class="None">
				<div class="menuName">음식이름 <input class="nameInput" placeholder="음식이름"/></div><!-- 음식이름 -->
				<div class="menuPrice">음식가격 <input class="priceInput" placeholder="음식가격"/></div><!-- 음식가격 -->
				</div>
			</div>
			<button id="button" class="None"><i class="fa fa-check-square"></i></button>
		</div><!-- 메뉴박스 end -->
...
$("#stamp").click(function(){	
	...
	$("#menus>#addBtn").click(function () {
			$("#menuAddBox").removeClass("None");
	    	$("#button").removeClass("None");
	});
	
	//메뉴입력
	$("#button>i.fa-check-square").on("click",function () {
		//메뉴 입력까지 성공!!
		//But) 만약 이미 등록되어있는 메뉴라면 update시켜! 2차개발..
		var menuName = $(".nameInput").val();
		var menuPrice = $(".priceInput").val();
		alert(menuName);
		alert(menuPrice);
		$.ajax("ajax/foodMenuInfo.jsp",{
				type:'post',
				dataType:"json",
				error:function(){
					alert("에러");
				},
				data:{contentNo:<%=no%>, name:menuName, price:menuPrice},
				success:function(json) {
					if(json.result){
						$("#menuAddBox").addClass("None");
						
						var $li = $("<li>");
						$li.append("<span>"+menuName+"</span> <span id='aa' class=''>"+menuPrice+"원</span>").prependTo(".menuList");
						$("#aa").addClass("mP");
						alert("성공!");
					}
				}
