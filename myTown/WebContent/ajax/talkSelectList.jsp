...
<%
	...
	Map<String, Object> talkList = new HashMap<>();
	...
	if(category.equals("전체")) {
		talkList.put("list",TownstalksDAO.talkList(pageVo));
		total = TownstalksDAO.selectTotal(addressNo);
	}else {
		talkList.put("list",TownstalksDAO.talkListByCategory(pageVo));
		total = TownstalksDAO.selectTotal(pageVo);
	}
	int numBlock = 10;
	
	talkList.put("page",PaginateUtilsim.paginate(now, total, numPage, numBlock, "talkList.jsp", "page="));
	
	ObjectMapper talkOm = new ObjectMapper();
	String json = talkOm.writeValueAsString(talkList);
